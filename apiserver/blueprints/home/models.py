...
import json, random
import click
from apiserver.utils import get_datetime, clean, clean_concat, change_if_date, HOST_IP
import pandas as pd
import numpy as np
import os
        ...
        if 'class_name' in kwargs.keys():
            ...
            for k in kwargs.keys():
                if list(kwargs.keys())[-1] == k:
                    ...
                    values = values + ", '{value}')".format(value=str(kwargs[k]).replace("'", "\\'"))
                else:
                    labels = labels + ", {label}".format(label=k)
                    values = values + ", '{value}'".format(value=str(kwargs[k]).replace("'", "\\'"))
                if k == 'icon':
...
class OdbServer():
    ...
    def __init__(self):
        ...
        self.path = os.getcwd()
        self.data = os.path.join(self.path, "data")
        self.demo = os.path.join(self.data, "netgraph.json")
    ...
    def create_db(self, db_name, db_type):
        ...
        self.client.batch('''
            create property V.key string;
            create property V.description string;
            create index V_key on V (con_id) UNIQUE;
            ...
            create property Event.startDate datetime;
            create property Event.endDate datetime;
    ...
    def get_db_index(self, db_name, limit=None):
        ...
        if db_name in self.databases:
            ...
            if isinstance(limit, int):
                try:
                    r = self.client.command('''select key from V limit %d''' % limit)
                # If there it fails it means this is a db_name with no key
                except Exception as e:
                    if str(e) == 'key':
                        self.client.batch('''
                            create property V.key string;
                            create index key on V (key) UNIQUE;
                            create sequence idseq type ordered;
                        ''')
            else:
                r = self.client.command('''select key from V''')
            for c in r:
                if 'key' in c.oRecordData.keys():
                    if c.oRecordData not in self.db_cache[db_name]:
                        self.db_cache[db_name].append(c.oRecordData['key'])
    ...
    def fill_demo_data_small(self):
        ...
        for db in self.databases:
            ...
            for e in latestEvents:
                ...
                if le_key not in graph['nodekeys']:
                    graph['nodekeys'].append(le_key)
                    graph['nodes'].append(
                        self.format_node(key=le_key,
                ...
                sql = '''match {class: Event, as: u, where:( key ='%s')}.both(){class: V, as: e} 
                        ...
                        e.icon, e.key,
                        u.CreateDate, u.Description''' % le_key
                for r in self.client.command(sql):
                    ...
                    if r['e_key'] not in graph['nodekeys']:
                        graph['nodekeys'].append(r['e_key'])
                        attributes = []
                        ...
                        graph['lines'].append({'from': le_key, 'to': r['e_key']})
                ...
                if len(kwargs['search_terms']) > 1:
                    ...
                    for st in search_terms:
                        if i == len(search_terms):
                            sql = sql + "key containstext '%s' " % str(st).lower()
                        else:
                            sql = sql + "key containstext '%s' or " % str(st).lower()
                            i+=1
                    try:
                        r = self.client.command(sql)
                    except Exception as e:
    ...
    def db_class_check(self, db_name, class_name):
        ...
        try:
            self.client.batch('''
                ...
                create property %s.key string;
                create index %key on %s (key) UNIQUE; 
